...
mod test {
    ...
    fn deserialize_clean_str() {
        ...
        for i in 0..3 {
            handles.push((
                asset_server.load(&format!("Zombie1/zombie1_{:05}.png", i)),
                (i + 1) as f32 / 10.,
            ));
        }
...
impl<A, M: AnimationMedium<Frame = A>> AnimationNode<A> for VariableNode<M> {
    ...
    fn run(&self, state: &mut AnimationState) -> NodeResult<A> {
        ...
        let mut frame_time =
            state.get_attribute::<f32>(&Attribute::LAST_FPS) * frames as f32 + rem_time;
                ...
                let (path, time) = {
                    ...
                    let time = paths
                        ...
                        .ok_or(Error::DeserializeError {
                            node_type: "VariableNode",
                            message: "Frame Time not found".into(),
                            loc: path.to_string(),
                        })?
                        .trim();
                    println!("{}", time);
                    let time = time[..time.len() - 1]
                        .parse::<f32>()
                        .or_else(|e| Err(Error::ParseFloatError(e)))?;
                    (&path[1..], time)
                };
