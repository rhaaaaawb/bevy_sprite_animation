...
impl<T> MatchType for T where T: 'static + Send + Sync + Eq + std::hash::Hash {}
...
mod loader {
    ...
    use crate::error::BevySpriteAnimationError as Error;
    use crate::prelude::{Attribute, NodeID};
        ...
        T: MatchType + std::any::Any + serde::de::DeserializeOwned + serde::Serialize + Ord,
    {
        ...
        ) -> Result<Box<dyn crate::prelude::AnimationNode<A>>, crate::error::BevySpriteAnimationError>
        {
            use std::collections::HashMap;
            let data = data.trim();
            let data = if data.starts_with(&format!("{}(", self.can_load[0])) {
                &data[self.can_load[0].len()..].trim()
            } else {
                data
            };
            ...
            let check = map.get("check").ok_or(Error::DeserializeError {
                node_type: "MatchNode",
                message: format!("Failed to find check: Attribute"),
                loc: crate::here!(),
            })?;
            let pairs = map.get("pairs").ok_or(Error::DeserializeError {
                node_type: "MatchNode",
                message: format!(
                    "Failed to find pairs: [({},NodeID)]",
                    std::any::type_name::<T>()
                ),
                loc: crate::here!(),
            })?;
            let default = map.get("default").ok_or(Error::DeserializeError {
                node_type: "MatchNode",
                message: format!("Failed to find default: {}]", std::any::type_name::<T>()),
                loc: crate::here!(),
            })?;
            let name = map.get("name").ok_or(Error::DeserializeError {
                node_type: "MatchNode",
                message: format!("Failed to find name: String]"),
                loc: crate::here!(),
            })?;
            let check: Attribute = ron::from_str(check)?;
            let pairs: Vec<(T, NodeID)> = ron::from_str(pairs).or_else(|_| {
                Err(Error::DeserializeError {
                    node_type: "MatchNode",
                    message: format!("Failed to deserialize pairs, check type matches"),
                    loc: crate::here!(),
                })
            })?;
