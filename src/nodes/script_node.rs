use crate::{
    prelude::{AnimationNode, Attribute, BevySpriteAnimationError, NodeID, NodeResult},
...
impl AnimationNode for ScriptNode {
    ...
    fn ui(
        &mut self,
        _ui: &mut bevy_inspector_egui::egui::Ui,
        ...
        while let Some(word) = words.next() {
            ...
            if word.starts_with("Ron(") {
                ...
                while ron.matches('(').collect::<Vec<&str>>().len()
                    != ron.matches(')').collect::<Vec<&str>>().len() - 1
                {
            ...
            let token = match word {
                ...
                "set" => Token::Set,
                "none" => Token::None,
                "return" => Token::Return(NodeID::from_str(
                    words.next().expect("NodeID to follow return"),
                )),
...
mod serialize {
    ...
    use crate::{
        node_core::CanLoad,
        prelude::{AnimationNode, NodeLoader},
    };
    use bevy::prelude::AssetServer;
            ...
            let data = if data.starts_with("ScriptNode(") {
                if data.ends_with("),") {
                    ...
                    return Err(Error::MalformedStr {
                        message: format!("Found ScriptNode( but failed to find Closing ')'"),
                        location: crate::here!(),
                    });
                }
            } else {
