...
impl Hash for Attribute {
    fn hash<H: Hasher>(&self, state: &mut H) {
        match self {
            Attribute::Index(id, _) | Attribute::IndexId(id) => {
                state.write_u8(u8::MAX - 1);
                state.write_u64(*id)
            }
            Attribute::CustomId(id) | Attribute::Custom(id, _) => {
...
impl Attribute {
    ...
    pub fn from_str(s: &str) -> Attribute {
        match ron::from_str::<Attribute>(s) {
            Ok(ok) => ok,
            Err(_) => Attribute::Custom(get_node_hash(&s), s.to_string().into()),
        }
    }
}
...
impl std::fmt::Display for Attribute {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            Attribute::Custom(_, name) | Attribute::Index(_, name) => {
                f.write_str("::")?;
                f.write_str(name.as_ref())
            }
            Attribute::CustomId(_) | Attribute::IndexId(_) => {
                f.write_str("(")?;
...
impl Attribute {
    ...
    pub fn name(&self) -> Option<&str> {
        match self {
            Attribute::Custom(_, name) | Attribute::Index(_, name) => Some(name.as_ref()),
            Attribute::CustomId(_) | Attribute::IndexId(_) => None,
            e => Some(e.as_ref()),
        }
    }
    ...
    pub fn name_or_id(&self) -> Cow<'_, str> {
        if let Some(name) = self.name() {
            ...
            match self {
                Attribute::CustomId(id) | Attribute::IndexId(id) => format!("{}", id).into(),
        ...
        S: serde::Serializer,
    {
        ...
        match self {
            Attribute::Custom(_, name) => {
                serializer.serialize_newtype_variant(SERDE_NAME, variant_index, variant_name, name)
            }
            Attribute::CustomId(id) => {
                serializer.serialize_newtype_variant(SERDE_NAME, variant_index, variant_name, id)
            }
...
#[derive(
    Debug, Clone, Copy, serde::Deserialize, strum_macros::AsRefStr, strum_macros::IntoStaticStr,
)]
enum Variant {
    Custom,
    Default,
